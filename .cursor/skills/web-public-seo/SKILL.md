...
6. **Ảnh:** lúc **tạo/sửa lô** (upload + Lưu title/địa chỉ) CDN key `{slug-ten-dia-chi}-anh-n.webp` (server convert WebP). **Đăng web không làm lại SEO ảnh.** **Bài CMS** (modal Soạn bài): `{slug-tieu-de}-anh-n.webp` dưới `public-web/`; bài cũ UUID → `pnpm images:seo-copy-posts`. `alt` guest = tiêu đề — không thêm ô Alt/Title. Ảnh chat **mới từ extension** → WebP lúc ingest (`customers/chat/`). JPEG/PNG chat+avatar cũ: `pnpm images:webp-replace` (ghi WebP, cập nhật DB, rồi xóa nguồn). Ảnh chat gắn lô: **copy** sang key SEO WebP. Ảnh dự án: `{ten-du-an}-anh-n.webp`. Kho JPEG/PNG lô cũ: `pnpm images:seo-copy`. Trang khách: `alt`; sitemap `images`; JSON-LD `ImageObject`. Gallery SSR đủ URL. Không chặn Googlebot trên CDN.
