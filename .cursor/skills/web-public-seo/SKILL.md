...
6. **Ảnh:** lúc **tạo/upload lô** server đặt CDN key `{slug-ten-dia-chi}-anh-n`. Ảnh **dự án**: `{ten-du-an}-anh-n` (`Address.detail`); alt trang khách = tên dự án. Kho cũ: `pnpm images:seo-copy` (lô) / `pnpm images:seo-copy-addresses` (địa chỉ PROJECT còn UUID); **bỏ qua** `customers/chat/` — [`PUBLIC-SEO.md`](../../../docs/PUBLIC-SEO.md) §10. Trang khách: `alt`; sitemap `images`; JSON-LD `ImageObject`. Gallery SSR đủ URL. Không chặn Googlebot trên CDN.
