...
6. **Ảnh:** lúc **tạo/sửa lô** (upload + Lưu title/địa chỉ) CDN key `{slug-ten-dia-chi}-anh-n.webp` (server convert WebP) **và** sibling `….og.jpg` (hoặc `….og.png`) cho `og:image`. Meta `og:image` nên qua **cùng origin** `/og-media/…` (proxy CDN) — Zalo gãy ảnh trên `cdn.*` trong khi `/og-default.png` ổn. **Đăng web không làm lại SEO ảnh.**
