...
6. **Ảnh:** lúc **tạo/upload lô** server đặt CDN key `{slug-ten-dia-chi}-anh-n`. Kho cũ: copy (không rename) bằng `pnpm images:seo-copy` — [`PUBLIC-SEO.md`](../../../docs/PUBLIC-SEO.md) §10. Trang khách: `alt`; sitemap `images`; JSON-LD `ImageObject`. Gallery SSR đủ URL. Không chặn Googlebot trên CDN.
