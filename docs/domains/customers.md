...
6. **Lưu chăm sóc** — cập nhật **đúng dòng đang đứng**, không `invalidate` list (API `updatedAt` sẽ kéo khách lên đầu nếu refetch). Máy tính: chụp `scrollTop` **trước khi** mở modal, khôi phục sau Lưu/Huỷ (đóng dialog không được kéo bảng về đầu). Điện thoại: trang `/cham-soc` vá cache rồi về list; nhớ cuộn như mục dưới. NV làm khách kế tiếp không phải cuộn lại.
