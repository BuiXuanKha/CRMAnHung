...
27. **Inbox Facebook sống** — hội thoại Meta realtime **trong** CRM (CRM cũ: extension quét `business.facebook.com/latest/inbox`). **Chưa làm.** Không nhầm với mục 21. Cần bàn: iframe / cửa sổ phụ / chỉ dựa extension + tin đã lưu. Scanner load `apps/extension/unpacked`.
...
Scanner Chrome: Load unpacked `apps/extension/unpacked` (cùng panel Business Suite / Messenger / E2EE). Gửi `POST /api/v1/customers/from-extension`. Stub TypeScript `apps/extension/src` chưa thay unpacked.
