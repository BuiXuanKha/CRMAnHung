...
Upload **mới** từ máy: object key CDN = `{slug tên+địa chỉ}-anh-{n}.webp` (API convert WebP). Ảnh gắn từ chat: **copy** sang key lô SEO WebP, **giữ** file `customers/chat/` (extension ingest = WebP; JPEG migrate giữ đến khi gắn lô; cùng ảnh chat trên hai lô → hai bản SEO). Ảnh **dự án**: `{tên dự án}-anh-{n}.webp` từ `Address.detail`.
