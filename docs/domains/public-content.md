...
| Slug ổn định | Slug = `toListingPublicSlug(title, leftover location)` lúc **tạo** listing (không trần 80; cấm `xa`). `uniqueSlug` **không** cấp chuỗi đang là `PublicLotSlugRedirect.fromSlug` (BUG-067): trùng thì thêm `-2` / `-3` vào **cuối** URL. `PATCH draft` / Đăng lại **không** đổi slug trừ khi NV sửa ô slug |
