...
- **SEO ảnh:** lúc upload, CDN key = slug tên lô + địa chỉ (`…-anh-1.webp`, file WebP); bài CMS: `public-web/{slug-tieu-de}-anh-n.webp`, `alt` = tiêu đề bài. Trang khách: `alt` + sitemap `image:loc` + JSON-LD `ImageObject` — [`PUBLIC-SEO.md`](./PUBLIC-SEO.md) §10.
