...
| Menu | **Công việc** \| Quản lý khách hàng \| Quản lý lô đất \| Quản lý giao dịch \| Dịch vụ sổ đỏ (+ Dashboard / quản trị nếu ADMIN) |
...
| 2026-08-26 | `/lo-dat`: giữa = lô NV đang Mở bán; phải = preview bài đăng trang khách |
| 2026-08-26 | `/lo-dat`: không H1, không nút Đăng lô cạnh ô tìm; ô tìm chrome như `/lo-dat`; preview ~440px; cột bảng sát list lô NV |
| 2026-08-26 | `/lo-dat` preview: **Đăng web** khi chờ đăng; lô đang hiện chỉ còn link xem trang khách — không nút Gỡ web |
| 2026-08-26 | Menu trong Dashboard: **Lô đất** (trước: Lô đất public mở bán) |
| 2026-08-26 | `/lo-dat` panel phải: tiêu đề **Preview Post** |
| 2026-08-26 | `/lo-dat`: lọc cột §4.5.5 (ảnh, địa chỉ, phân loại, DT/hướng, giá, NV, Web); mobile Bộ lọc |
...
| 2026-09-02 | STAFF soạn bài đăng trên `/lo-dat` (lô mình tạo). Preview **Gỡ web** khi đang hiện. Bài CMS + Tổng quan vẫn ADMIN |
...
| 2026-09-10 | Chi tiết `/lo-dat/[id]` mobile FAB «⋯»: thêm **AI GPT** (cùng modal list; STAFF only) |
...
| 2026-09-12 | Chi tiết `/giao-dich/[id]` mobile: ẩn «← Quản lý giao dịch» + nút Sửa header; FAB 「⋯」 (công việc · sửa · xóa); máy tính không đổi |
| 2026-09-12 | Chi tiết `/giao-dich/[id]`: hero thẻ trắng, nhãn không IN HOA dày, giá bán nhấn; ảnh snapshot mở `LodatImageGallery` (không tab mới) |
| 2026-09-10 | Chi tiết `/lo-dat/[id]`: hangtag **Đã soạn** / **Chưa soạn** (cột Web list) + icon đỏ `needsWebUpdate` |
| 2026-09-10 | `/lo-dat` cột Web + nhãn Đã/Chưa soạn — đồng nhất list/chi tiết (bỏ «Đã đăng web» / Đã·Chưa đăng) |
...
| 2026-09-08 | Dashboard chỉ ADMIN; STAFF soạn listing lô trên `/lo-dat` |
...
| 2026-09-09 | `/lo-dat` Soạn bài đăng: mặc định **Giá gợi ý**; nút **Sinh giá gợi ý** từ CRM |
