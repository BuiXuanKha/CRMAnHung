...
| Web shell CRM `(crm)/*` | Ready | login API + list khách / lô / GD / sổ đỏ + khung `/cong-viec` |
...
3. **Cấu hình `anhungland.com`:** làm theo [`ANHUNGLAND-SETUP.md`](./ANHUNGLAND-SETUP.md)
   ...
   - ~~Secrets GitHub `DEPLOY_SSH_HOST` + `DEPLOY_SSH_KEY`~~ — đã gắn; deploy = skill `deploy-staging` (gộp `main` → Actions tự chạy)
